window.saveFormData = function(displayNotify = false, reload = false){
    ...
    let uri = route('clientrequests.car.validateandsave');
    ...
    if($('.unl_on_select')[0].selectedOptions[0].value === '4'){
        $(".car-slider-weight").val(totalWeight);
        $(".car-slider-volume").val(totalVolume);
        enableCarSlider({totalWeight: totalWeight, totalVolume: totalVolume});
    }
...
$(function(){
    let checkKtkSize = () => {
        let value = $('.car_size_select')[0].selectedOptions[0].value;
        if(value.length > 0){
            $.ajax({
                url: route('clientrequests.car.getktktypeweightdata'),
                type: 'post',
                data: {
                    type: value
                },
                success: (res) => {
                    res = JSON.parse(res);
                    $('.car-slider-weight')[0].dataset.weight = $('.car-slider-volume')[0].dataset.weight = res.weight;
                    $('.car-slider-weight')[0].dataset.volume = $('.car-slider-volume')[0].dataset.volume = res.volume;
                    enableCarSlider({weight: res.weight, volume: res.volume});
                    $('.car_spoiler_group').removeClass('d-none');
                },
                error: () => {
            ...
            $('.car_spoiler_group').addClass('d-none');
        }
    };
    checkKtkSize();

    $(document).on('change', '.car_size_select', function(){
        checkKtkSize();
    });

    window.enableCarSlider = function(options = {}){
        if($(".car-slider-weight").length > 0){
            let weightInput = $(".car-slider-weight")[0];
            $(".car-slider-weight").ionRangeSlider({
                block: true,
                skin: "round",
                type: "single",
                min: 0.01,
                ...
                onChange: function (data) {
                    ...
                    carVolumeSliderArray.update({
                        from: newVolumeValue,
                        to: newVolumeValue
                    });
                },
            });
            carWeightSliderArray = $(".car-slider-weight").data("ionRangeSlider");
        }

        if($(".car-slider-volume").length > 0){
            let volumeInput = $(".car-slider-volume")[0];
            $(".car-slider-volume").ionRangeSlider({
                block: true,
                skin: "round",
                type: "single",
                min: 0.01,
                ...
                onChange: function (data) {
                    ...
                    carWeightSliderArray.update({
                        from: newWeightValue,
                        to: newWeightValue
                    });
                },
            });
            carVolumeSliderArray = $(".car-slider-volume").data("ionRangeSlider");
        }
    ...
    var weightSliderArray = [],
        volumeSliderArray = [],
        amountSliderArray = [],
        carWeightSliderArray = [],
        carVolumeSliderArray = [],
        clId = [];
    ...
    $(document).on('click', '.open_cargos_model', function(e){
        ...
        if(enabled.length > 0){
            $.ajax({
                type: 'post',
                url: route('clientrequests.car.getimportform'),
                data: {
    ...
    $(document).on('click', '.submit_cargo_export', function (e) {
        ...
        $.ajax({
            url: route('clientrequests.car.importcargo'),
            type: 'post',
            data: {
                id: clId,
                form: form
    ...
    function setOrderValue(orderName, value){
        let orderTd = $('td[data-type="'+orderName+'"]');
    ...
    function checkAb(){
        ...
        if(firstVal > 0 && secondVal > 0){
           ...
           // $('.client_request_set_order_status').last().removeClass('d-none').attr('data-value', tableRes);
            ...
            let url = route('clientrequests.car.pickorders'),
            value = {
                type: tableRes
            };
 ...
 //   console.log(orderRadioButtonFromForm);
    ...
    $(document).on('change', '.unl_on_select', function(){
        let val = this.selectedOptions[0].value,
            fromTo = $(this).data('fromto');
        console.log(val, fromTo, '.clientrequest-'+fromTo+'-wrapper');
        if(val !== ''){
    ...
    function updateProviderFtlCheckbox(){
        let fromBlocksCount = $('.clientrequest-from-form-wrapper').length;
        setOrderValue(orders.car.providerClient, fromBlocksCount);
    }
    ...
    function getJobStatusProgress(statusId){
        ...
        if(requestEnabled === true){
            var tes = setInterval(() => {
                $.ajax({
                    type: 'get',
                    url: route('clientrequests.car.getjobstatusprogress', statusId),
                    success: (res) => {
    ...
    $(document).on('click', '.create_orders_from_cl_request', function(e){
        ...
        let uri = route('clientrequests.car.createorders'),
            _this = $(this);
    ...
    $(document).on("click", ".add_clrqstfrom_form-js", function(e){
        ...
        $(wrapper).find('.clientrequest-from-form-wrapper').last().find('.from_select option:last').remove();
        updateProviderFtlCheckbox();
        updateFunctions();
    });
    ...
    $(document).on("click", ".add_clrqstto_form-js", function(e){
        ...
        var html = tmpl.render(data);
        _this.before(html);
        $(wrapper).find('.clientrequest-to-form-wrapper').last().find('.to_select option:last').remove();
        updateFunctions();
    });
