...
namespace App\Jobs\ClientRequest\Car;
...
class CreateOrders implements ShouldQueue
{
    ...
    {
        $this->detachOldRelations(optional($this->client)->lead);
        $data = $this->setBlocksDataFormRequest($this->request['clientrequest']);
        //$this->client->warehouseCargos()->delete();

//        if(!empty($this->client->products()->get()->toArray()) && !empty( $this->client->lead->clients)){
//            foreach ($this->client->lead->clients as $leadClient) {
//                $warehouseCargoRepository->syncWithClientRequestProducts($this->client->products()->get()->toArray(), GettingActCargo::IN_PROCESS_STATUS, $leadClient->id);
//            }
//        }
        ...
        foreach ($ordersToCreate as $item) {
            switch ($item){
                Case Order::CAR_PROVIDER_CLIENT_NAME:
                    for ($i = 0; $i < intval($ordersCount[Order::CAR_PROVIDER_CLIENT_NAME]); $i++) {
                        $count++;
                        $this->setProgressNow($count);
                        $order = $orderRepository->create(
                            Order::CAR_TYPE,
                            Order::CAR_PROVIDER_CLIENT_NAME,
                            Order::orderIndex()[Order::CAR_PROVIDER_CLIENT_NAME].' '.$this->request['lead_id'].'-'.($i + 1),
                        ...
                        $this->createCarReports($order->id, $date);
                    }
                    break;
                ...
                case Order::TR_NAME:
                    for ($i = 0; $i < intval($ordersCount[Order::TR_NAME]); $i++){
                        ...
                        $this->createTrainReports($order->id, $date);
                    }
                break;
     ...
     * @throws \Exception
     */
    private function createCarReports($orderId, $date)
    {
        for($i = 0; $i < 60; $i++){
            if($i > 0){
     ...
     * @throws \Exception
     */
    private function createTrainReports($orderId, $date)
    {
        for($i = 0; $i < 60; $i++){
            if($i > 0){
                $date = new \DateTime($date);
