...
@section('title', 'FTL: ЖД Сделки')
@section('content_header')
    ЖД Сделки @include('partials.title', ['text' => App\Title::get()['leadIndex']['lead']])
@stop
    ...
    <div class="row">
        <div class="kt-portlet__body">
            @permission('lead_create')
                <a role="button" class="btn btn-success mb-3 ml-3" href="{{route('leads.tr.create')}}">Создать ЖД Сделку</a>
            @endpermission
        </div>
    </div>
    <div class="row">
            ...
            <div class="table-responsive">
                <table class="table table-hover table-striped table-borderes leads_table">
                    ...
                    @foreach($leads as $lead)
                        <tr onclick="location.href ='{{route('leads.tr.edit', ['lead' => $lead])}}'">
                            <td>{{ $lead->id }}</td>
