...
@stop
@push('head')
    <script src="{{ asset('js/clientrequests/car.js') }}"></script>
...
{{--                    {{ dd($leadClient->inn) }}--}}
            ...
            <form enctype="multipart/form-data" id="clientrequestsform" action="{{route('clientrequests.car.update', ['client' => $client])}}" method="post">
                @csrf
                {{ method_field('put') }}

                <div class="col-md-12 pl0">
                    @permission('client_request_edit')
                        @include('clientrequests.form', ['lead' => $lead, 'client' => $client])
                    @endpermission
                </div>
                ...
                <div class="col-sm-12 pl0">
                    ...
                    <div class="form-group">
                        <a href="{{ route('leads.car.edit', ['lead' => $lead]) }}" class="btn btn-primary back_to_lead_link">Вернуться в Сделку</a>
                    </div>
...
{{--                @include('clientrequests.ordersModal')--}}
            ...
            <div class="col-sm-12 pl0">
                @if($userManager->getId() == $lead->responsible_user_id || $userManager->hasRole('admin'))
                    <form action="{{ route('clientrequests.car.destroy', ['client' => $client]) }}" method="post">
                        @csrf
                        {{ method_field('delete') }}
