...
<table class="table table-hover table-striped table-bordered tr_orders_block">
    ...
    <tbody>
    <tr>
        <td data-type="{{App\Models\Entities\Order::CAR_PROVIDER_CLIENT_NAME}}">
            <label><input value="{{App\Models\Entities\Order::CAR_PROVIDER_CLIENT_NAME}}" type="checkbox" {{ in_array(App\Models\Entities\Order::CAR_PROVIDER_CLIENT_NAME, $excludedOrders) ? 'checked' : '' }} class="jtoggler orderstocreate_checkbox" name="clientrequest[orderstocreate][]">Авто Заявка: Поставщик - Клиент</label>
            <div class="row">
                <label class="control-label col-8">Кол-во заявок</label>
                <input class="form-control col-4 orderstocreate_count" type="number" min="0" value="{{ in_array(App\Models\Entities\Order::CAR_PROVIDER_CLIENT_NAME, $excludedOrders) ? '1' : '0' }}" name="clientrequest[countorderstocreate][{{App\Models\Entities\Order::CAR_PROVIDER_CLIENT_NAME}}]">
            </div>
        </td>
        <td></td>
    </tr>
