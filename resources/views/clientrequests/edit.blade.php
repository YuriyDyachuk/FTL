...
@stop
@push('head')
    <script src="{{ asset('js/clientrequests.js') }}"></script>
...
{{--                    {{ dd($leadClient->inn) }}--}}
            ...
            <form enctype="multipart/form-data" id="clientrequestsform" action="{{route('clientrequests.update', ['client' => $client])}}" method="post">
                @csrf
                {{ method_field('put') }}
                ...
                <div class="col-sm-12 pl0">
                    ...
                    <div class="form-group">
                        <a href="{{ route('leads.edit', ['lead' => $lead]) }}" class="btn btn-primary back_to_lead_link">Вернуться в Сделку</a>
                    </div>
            ...
            <div class="col-sm-12 pl0">
                @if($userManager->getId() == $lead->responsible_user_id || $userManager->hasRole('admin'))
                    <form action="{{ route('clientrequests.destroy', ['client' => $client]) }}" method="post">
                        @csrf
                        {{ method_field('delete') }}
