<h2>Груз</h2>
<a href="#" class="btn btn-primary cargo_import_to_client_request" data-client-ids="{{ json_encode($lead->getClientsIds()) }}" data-id="{{ $client->id }}">Добавить Груз</a>
<div class="under_line"></div>
@include('clientrequests.products.productTemplate', ['lead' => $lead])
{{--@include('clientrequests.products.dtTemplate')--}}

<div class="product-wrapper">
    @if($lead->goods()->exists())
        @foreach($lead->goods as $n => $product)
            <div data-product-id="{{ $product->id }}" style="position: relative;" class="product-form-wrapper {{ $product->from_warehouse_cargo ? 'from_warehouse_cargo' : '' }}" data-i="{{$n}}">
                <div class="row">
                    <div class="col-md-12">
{{--                        <input type="hidden" name="product[{{$n}}][client_request_id]" value="{{ $client->id }}">--}}
                        <div class="row">
                            <input name="product[{{$n}}][from_warehouse_cargo]" type="hidden" value="{{ $product->from_warehouse_cargo }}">
                            <div class="col-md-6">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control" name="product[{{$n}}][client_id]">
                                                        ...
                                                        @foreach(\App\Models\Entities\Client::getList() as $id => $name)
                                                            @if(!in_array($id, $lead->clients()->pluck('id')->toArray()))
                                                                @continue
                                                            @endif
...
{{--                                    <input class="uid_input" type="hidden" name="product[{{$n}}][uid]" value="{{ $product['uid'] }}">--}}
                                    <div class="col-6">
                                    ...
                                    <div class="col-3 form-group field-productform-{{$n}}-weight">
                                        ...
                                        <input value="{{ $product['weight'] }}" class="form-control cargo_amount goods_weight" type="text" name="product[{{$n}}][weight]" id="productform-{{$n}}-weight">
                                        <div class="help-block"></div>
                                    </div>
                                    <div class="col-3 form-group field-productform-{{$n}}-volume">
                                        ...
                                        <input value="{{ $product['volume'] }}" class="form-control cargo_amount goods_volume" type="text" name="product[{{$n}}][volume]" id="productform-{{$n}}-volume">
                                        <div class="help-block"></div>
                                    </div>
                                </div>
                            </div>
    ...
    @else
        <div style="position: relative;" class="product-form-wrapper" data-i="0">
            <div class="row">
                <div class="col-md-12">
{{--                    <input type="hidden" name="product[0][client_request_id]" value="{{ $client->id }}">--}}
                    <div class="row">
                        <input name="product[0][from_warehouse_cargo]" type="hidden">
                        <div class="col-md-6">
                            <div class="product-dt-wrapper">
                                <div style="position: relative;" class="dt-form-wrapper">
                                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="row">
                                <input type="hidden" name="product[0][status]" value="{{ \App\Models\Entities\GettingActCargo::IN_THE_WAREHOUSE_STATUS }}">
{{--                                <input class="uid_input" type="hidden" name="product[0][uid]" value="{{ \Str::random(32) }}">--}}
                                <div class="col-6">
                                ...
                                <div class="col-3 form-group field-productform-0-weight">
                                    ...
                                    <input class="form-control cargo_amount goods_weight" type="text" name="product[0][weight]" id="productform-0-weight">
                                    <div class="help-block"></div>
                                </div>
                                <div class="col-3 form-group field-productform-0-volume">
                                    <label for="productform-0-volume" class="control-label">Объём, м3</label>
                                    <input class="form-control cargo_amount goods_volume" type="text" name="product[0][volume]" id="productform-0-volume">
                                    <div class="help-block"></div>
                                </div>
                            </div>
                        </div>
